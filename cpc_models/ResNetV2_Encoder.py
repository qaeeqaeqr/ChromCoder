...
class PreActResNet_Encoder(nn.Module):
    def __init__(self, args, use_classifier, block, num_blocks, num_channels):
        ...
        # Stem Net
        # for chromosome data, the first convolution layer may be changed
    ...
    def forward(self, x):
        # Input x = (batch_size, grid_size, 1, channels, patch_size, patch_size)
        grid_size = self.args.grid_size

        # Flatten to (batch_size * grid_size * 1, channels, patch_size, patch_size)
        x = x.view(
        ...
        z = self.avgpool(z)
        # NOTE: for CMS_patched chromosome image, the third param. below should be 1 (other columns are empty)
        z = z.view(-1, grid_size, 1, z.shape[1])  # (batch_size, grid_size, 1, encoding_size)

        # Use classifier if specified
        if self.use_classifier:
            # Reshape z to (batch_size, grid_size * grid_size, encoding_size)
            # due to the fact that only the first column is used, the second $grid_size$ is set to 1
            # z = z.view(z.shape[0], grid_size * 1, self.encoding_size)
            ...
            z = z.view(z.shape[0] * grid_size * 1, self.encoding_size)
            z = self.classifier(z)
            z = z.view(-1, grid_size * 1, self.args.num_classes)
            z = torch.mean(z, dim=1)
