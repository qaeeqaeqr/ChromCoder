...
def train():
    ...
    for epoch in range(args.trained_epochs+1, args.trained_epochs+args.epochs+1):
        ...
        for i, (batch, _) in enumerate(tqdm(unsupervised_loader, disable=args.print_option, dynamic_ncols=True)):
            net.zero_grad()
            loss = net(batch.to(args.device))  # 没有标签，将输出loss的任务直接分配给网络
...
def save(net, epochs):
    ...
    if not os.path.exists(os.path.join("TrainedModels", args.dataset)):
        os.mkdir(os.path.join("TrainedModels", args.dataset))
...
if __name__ == "__main__":
    ...
    if args.encoder[:6] == "resnet":
        ...
        enc = ViT_Encoder(args, use_classifier=False)
    else:
    ...
    # Original CPC inplementation doesnot contains dynamic learning rate
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=100, gamma=0.2)

    # Get selected dataset
    if args.dataset == "stl10":
        unsupervised_loader, _, _ = get_stl10_dataloader(args, is_labeled=False)
